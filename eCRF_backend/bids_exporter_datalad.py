...
VERSION_DIR_PREFIX = os.getenv("BIDS_VERSION_DIR_PREFIX", "v")
...
def _template_schema_path(dataset_path: str, version: int) -> str:
    return os.path.join(dataset_path, "canonical", "templates", f"v{int(version):03d}", "schema.json")


def _latest_template_version_from_canonical(dataset_path: str) -> int:
    templates_dir = os.path.join(dataset_path, "canonical", "templates")
...
def get_version_schema(study_id: int, study_name: str, version: Optional[int] = None) -> dict:
    dataset_path = _dataset_path(study_id, study_name)
    v = int(version) if version is not None else _latest_template_version_from_canonical(dataset_path)
    path = _template_schema_path(dataset_path, v)
    return _json_read(path, default={}) or {}


def latest_writable_version(study_id: int, study_name: str) -> int:
    dataset_path = _dataset_path(study_id, study_name)
    return _latest_template_version_from_canonical(dataset_path)


def assert_latest_is_used(study_id: int, study_name: str, version: Optional[int]) -> int:
    latest = latest_writable_version(study_id, study_name)
    if version is None:
        return latest
    try:
        req = int(version)
    ...
    if req != latest:
        raise ValueError(f"Only latest version is writable. Latest version is {latest}")
    return latest

# -------------------- One-time migration: root → version --------------------
...
def _migrate_root_contents_into_version(dataset_path: str, version: int) -> None:
    ...
    try:
        for name in os.listdir(dataset_path):
            ...
            if name in ("metadata", "canonical", LATEST_POINTER_NAME):
                continue
...
def upsert_bids_dataset(
    ...
    canonical_dir = os.path.join(dataset_path, "canonical")
...
def _value_from_entry_dict_like(entry_data: Any, sIdx: int, fIdx: int, study_data: dict):
    ...
    if not (0 <= sIdx < len(models)):
        return None
    sec = models[sIdx]
    sec_name_candidates = [_sec_title(sec), _normalize_token(_sec_title(sec))]
    sec_dict = None
...
def audit_change_both(
    *,
    ...
    if write_study_bids:
        try:
            _append_change_line(study_id=study_id, study_name=study_name, action=action, detail=payload)
        except Exception as e:
...
def write_entry_to_bids(
    ...
    *,
    ...
    schema_for_version = get_version_schema(study_id, study_name, form_version)
...
def stage_file_for_modalities(
    ...
    *,
    ...
    if subject_index is None and visit_index is None:
        ...
        if written:
            ...
            audit_change_both(
                ...
                actor_id=actor_id,
                actor_name=actor_name or actor,
                detail={"targets": [os.path.relpath(p, dataset_path) for p in written], "filename": filename or ""},
            )
    ...
    if form_version is not None:
        ...
        target_version = latest_writable_version(study_id, study_name)

    schema_for_version = get_version_schema(study_id, study_name, target_version)
    sd = schema_for_version or _json_clone(study_data if isinstance(study_data, dict) else {})
